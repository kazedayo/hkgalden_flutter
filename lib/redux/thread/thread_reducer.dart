...
ThreadState updateThreadReducer(ThreadState state, UpdateThreadAction action) {
  if (action.isInitialLoad == true) {
    return state.copyWith(threadIsLoading: false, thread: action.thread, isInitialLoad: false);
  } else {
    return state.copyWith(threadIsLoading: false, thread: state.thread.copyWith(replies: state.thread.replies..addAll(action.thread.replies), totalReplies: action.thread.totalReplies), isInitialLoad: false);
  }
}
