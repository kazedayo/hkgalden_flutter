...
class HKGaldenHtmlParser {
  ...
  Element _spanParsing(Element element) {
    switch (element.getAttribute('data-nodetype')) {
      ...
      //parse bold
      case 'b':
        return Element.tag('b')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse italic
      case 'i': 
        return Element.tag('i')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse underline
      case 'u':
        return Element.tag('u')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse strikethrough
      case 's':
        return Element.tag('s')..setInnerHtml(parse(element.innerHtml), validator: validator); 
        break;
      //parse center alignment
      case 'center':
        return Element.div()..setAttribute('class', 'center')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse right alignment
      case 'right':
        return Element.div()..setAttribute('class', 'right')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse h1
      case 'h1':
        return Element.tag('h1')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse h2
      case 'h2':
        return Element.tag('h2')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      //parse h3
      case 'h3':
        return Element.tag('h3')..setInnerHtml(parse(element.innerHtml), validator: validator);
        break;
      default:
        return element;
        break;
    ...
    if (rootParent != null && !store.state.sessionUserState.sessionUser.blockedUsers.contains(rootParent.author.userId)) {
      if (rootParent.parent != null && !store.state.sessionUserState.sessionUser.blockedUsers.contains(rootParent.parent.author.userId)) {
        if (rootParent.parent.parent != null && !store.state.sessionUserState.sessionUser.blockedUsers.contains(rootParent.parent.parent.author.userId)) {
          ...
          ${rootParent.content}</blockquote>${htmlDoc.body.innerHtml}''', validator: validator);
